...
static const ScenarioEntry g_scenario_entries[] = {
    ...
    {"long_soak", "Длительный тест устойчивости", "Прогрев + длительная нагрузка + телеметрия"},
...
static int token_has(const char *line, int idx)
{
  if (!line || !*line)
    return 1;
  char t[8];
  snprintf(t, sizeof(t), "%d", idx);
  return strstr(line, t) != NULL;
}
...
Scenario build_custom_scenario_from_prompt(void)
{
  ...
  fprintf(stdout, "\n[Manual] Выбери тесты (через пробел, Enter=все):\n");
  ...
  if (token_has(line, 4))
    s.steps[n++] = make_step("network_manual", WK_PING, 45, 1, "1.1.1.1", "steady", "Ручной network RTT");
  if (token_has(line, 5))
  ...
  if (n == 0)
    s.steps[n++] = make_step("cpu_fallback", WK_CPU_BURN, 60, 1, NULL, "steady", "Fallback CPU");
  ...
  fprintf(stdout, "\n[Manual] Noise mode: 0-none, 1-cpu, 2-io, 3-combined (Enter=0): ");
  if (read_line_stdin(line, sizeof(line)) == 0 && line[0] != '\0')
  {
    int nm = atoi(line);
    if (nm >= 0 && nm <= 3)
      s.noise_mode = (NoiseMode)nm;
  }
...
Scenario scenario_from_name(const char *name)
{
  ...
  {
    const char *metrics[] = {"cpu_ops_avg", "stability_score_pct", "temp_c_max", "psi_cpu_some_avg10_max", "jitter_p99_us"};
    s.name = "long_soak";
    ...
    s.steps[0] = make_step("warmup", WK_CPU_BURN, 180, 2, NULL, "burst", "Разогрев");
    s.steps[1] = make_step("steady_cpu", WK_CPU_BURN, 1200, 2, NULL, "steady", "Длинная CPU фаза");
    s.steps[2] = make_step("steady_mem", WK_MEMORY, 120, 1, "128M", "steady", "Memory фаза");
    s.steps[3] = make_step("steady_storage", WK_STORAGE, 240, 1, "4M", "steady", "Storage фаза");
    s.steps[4] = make_step("steady_net", WK_PING, 180, 1, "1.1.1.1", "steady", "Network фаза");
    s.steps[5] = make_step("steady_jitter", WK_JITTER, 30, 1, "1000", "steady", "Jitter фаза");
    s.step_count = 6;
    set_primary_metrics(&s, metrics, 5);
  }
  else if (strcmp(name, "server_gateway") == 0 || strcmp(name, "server_edge") == 0)
  {
    ...
    s.steps[0] = make_step("cpu_warmup", WK_CPU_BURN, 120, 4, NULL, "burst", "Разогрев CPU");
    s.steps[1] = make_step("storage_heavy", WK_STORAGE, 240, 1, "8M", "steady", "Тяжёлый storage");
    s.steps[2] = make_step("network_tail", WK_PING, 120, 1, "8.8.8.8", "steady", "RTT tail");
    s.steps[3] = make_step("jitter_check", WK_JITTER, 20, 1, "1000", "steady", "Проверка jitter");
    s.step_count = 4;
    set_primary_metrics(&s, metrics, 5);
  }
  else if (strcmp(name, "iot") == 0 || strcmp(name, "iot_controller") == 0)
  {
    ...
    s.steps[0] = make_step("idle_baseline", WK_IDLE, 60, 1, NULL, "recovery", "Фоновый baseline");
    s.steps[1] = make_step("cpu_burst", WK_CPU_BURN, 30, 1, NULL, "burst", "CPU burst");
    s.steps[2] = make_step("mem_burst", WK_MEMORY, 20, 1, "32M", "burst", "Memory burst");
    s.steps[3] = make_step("uplink_probe", WK_PING, 60, 1, "1.1.1.1", "steady", "Uplink probe");
  ...
  {
    const char *metrics[] = {"cpu_ops_avg", "jitter_p99_us", "temp_c_max", "stability_score_pct", "power_w_avg"};
    s.name = "embedded";
    ...
    s.steps[2] = make_step("control_mem", WK_MEMORY, 45, 1, "64M", "steady", "Контроль memory");
    s.steps[3] = make_step("control_jitter", WK_JITTER, 25, 1, "1000", "steady", "Контроль jitter");
    s.step_count = 4;
    set_primary_metrics(&s, metrics, 5);
  }
  ...
  else
  {
    ...
    s.steps[0] = make_step("cpu_base", WK_CPU_BURN, 60, 2, NULL, "steady", "CPU baseline");
    s.steps[1] = make_step("mem_base", WK_MEMORY, 30, 1, "64M", "steady", "Memory baseline");
    s.steps[2] = make_step("storage_base", WK_STORAGE, 60, 1, "4M", "steady", "Storage baseline");
    s.steps[3] = make_step("net_base", WK_PING, 45, 1, "1.1.1.1", "steady", "RTT baseline");
    s.steps[4] = make_step("jitter_base", WK_JITTER, 15, 1, "1000", "steady", "Jitter baseline");
    s.step_count = 5;
    set_primary_metrics(&s, metrics, 5);
  }
  ...
  while (1)
  {
    ...
    if (strcmp(line, "9") == 0)
    {
      ...
      *replace_latest = 0;
      ...
      fprintf(stdout, "\nSelected: run all scenarios\n");
      fprintf(stdout, "Duration scale (Enter = 0.05): ");
      if (read_line_stdin(line, sizeof(line)) != 0)
        return -1;
...
void print_execution_plan(const Scenario *sc)
{
  ...
  for (int i = 0; i < sc->step_count; ++i)
  {
    ...
    fprintf(stdout, "  %d) %s | %s | %ds | threads=%d | load=%s\n",
            i + 1, st->name, kind_name_local(st->kind), st->duration_sec, st->threads,
